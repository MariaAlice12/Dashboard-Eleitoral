...
async function fetchDeputadosStats(partido: string, uf: string): Promise<DeputadosStats> {
  const params = new URLSearchParams()
  if (partido && partido !== 'todos') params.set('siglaPartido', partido)
  if (uf && uf !== 'todos') params.set('siglaUf', uf)
  const res = await fetch(`/api/deputados/stats?${params}`)
...
export default function HomePage() {
  ...
  const { data: stats } = useQuery({
    queryKey: ['deputados-stats', partido, uf],
    queryFn: () => fetchDeputadosStats(partido, uf),
